...
cabin::cabin(QObject *parent) : QObject(parent)
{
    ...
    QObject::connect(this, SIGNAL(cabin_achieved_target(long)),
                     this, SLOT(stop(long)));
    QObject::connect(&_doors, SIGNAL(doors_closed()),
                     this, SLOT(wait()));
    QObject::connect(this, SIGNAL(cabin_stopped()),
                     &_doors, SLOT(start_opening()));
    QObject::connect(&cross_floor_timer, SIGNAL(timeout()),
                     this, SLOT(move()));
    QObject::connect(this, SIGNAL(cabin_called()),
                     this, SLOT(move()));
}
